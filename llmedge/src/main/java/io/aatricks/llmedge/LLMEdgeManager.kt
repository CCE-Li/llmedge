...
/**
 * High-level manager for LLMEdge operations.
 * Simplifies model loading, generation, and memory management.
 */
object LLMEdgeManager {
    private const val TAG = "LLMEdgeManager"
    private const val MIN_AVAILABLE_MEMORY_MB = 2000L

    // Default Model Configuration (Wan 2.1)
    private const val DEFAULT_MODEL_ID = "Comfy-Org/Wan_2.1_ComfyUI_repackaged"
    private const val DEFAULT_MODEL_FILENAME = "wan2.1_t2v_1.3B_fp16.safetensors"
    private const val DEFAULT_VAE_ID = "Comfy-Org/Wan_2.1_ComfyUI_repackaged"
    private const val DEFAULT_VAE_FILENAME = "wan_2.1_vae.safetensors"
    private const val DEFAULT_T5XXL_ID = "city96/umt5-xxl-encoder-gguf"
    private const val DEFAULT_T5XXL_FILENAME = "umt5-xxl-encoder-Q3_K_S.gguf"
    ...
    suspend fun generateText(
    ...
    ): String = loadMutex.withLock {
        contextRef = WeakReference(context.applicationContext)
        
        // Unload heavy diffusion models if loaded to free up memory
        unloadDiffusionModel()

        val smol = getOrLoadSmolLM(context, params.modelId, params.modelFilename, params.modelPath)
        
        // Reset and set system prompt if needed (omitted for simplicity as SmolLM API is stateful)
        ...
        if (onProgress != null) {
            ...
            return@withLock kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default) {
    ...
    suspend fun extractText(
        ...
        val result = engine.extractText(io.aatricks.llmedge.vision.ImageSource.BitmapSource(image))
    ...
    suspend fun analyzeImage(
        ...
        try {
            ...
            val tempImageFile = File.createTempFile("vision_input", ".jpg", context.cacheDir)
            val preparedImageFile = File.createTempFile("vision_prepared", ".bin", context.cacheDir)
            
            try {
                // Preprocess and save
                val scaled = io.aatricks.llmedge.vision.ImageUtils.preprocessImage(params.image, correctOrientation = true, maxDimension = 1024, enhance = false)
                tempImageFile.outputStream().use { out -> scaled.compress(Bitmap.CompressFormat.JPEG, 90, out) }
                
                // 2. Run Projector
                val projector = io.aatricks.llmedge.vision.Projector()
                projector.init(projFile.absolutePath, smol.getNativeModelPointer())
                val ok = projector.encodeImageToFile(tempImageFile.absolutePath, preparedImageFile.absolutePath)
                projector.close()
                
                val imageSource = if (ok) {
                    io.aatricks.llmedge.vision.ImageSource.FileSource(preparedImageFile)
                } else {
                    // Fallback
                    io.aatricks.llmedge.vision.ImageSource.FileSource(tempImageFile)
                }

                // 3. Run Analysis
                adapter.loadVisionModel(modelFile.absolutePath)
                val result = adapter.analyze(
                    imageSource, 
                    params.prompt, 
                ...
                adapter.close() // Close adapter but keep SmolLM cached? Or close both?
                // Adapter doesn't own SmolLM, so it's safe to close adapter.
            }
        } catch (e: Exception) {
    ...
    /**
     * Generates an image using the default or configured model.
     * Automatically handles sequential loading for low-memory devices.
     */
    suspend fun generateImage(
        context: Context,
        params: ImageGenerationParams,
        onProgress: ((String, Int, Int) -> Unit)? = null
    ): Bitmap? = loadMutex.withLock {
        contextRef = WeakReference(context.applicationContext)
        ...
        if (useSequential) {
            return generateImageSequentially(context, params, onProgress)
        } else {
            val model = getOrLoadModel(context, params.flashAttn, onProgress)
            val bytes = model.txt2img(
            ...
            return bytes?.let { android.graphics.BitmapFactory.decodeByteArray(it, 0, it.size) }
        }
    }
    ...
    suspend fun generateVideo(
        ...
        onProgress: ((String, Int, Int) -> Unit)? = null
    ): List<Bitmap> = loadMutex.withLock {
        contextRef = WeakReference(context.applicationContext)
        unloadSmolLM() // Free up memory from LLM
        ...
        if (useSequential) {
            ...
            val model = getOrLoadModel(context, params.flashAttn, onProgress)
            val sdParams = StableDiffusion.VideoGenerateParams(
                prompt = params.prompt,
                negative = params.negative,
                width = params.width,
                height = params.height,
            ...
            val progressWrapper = StableDiffusion.VideoProgressCallback { step, totalSteps, currentFrame, totalFrames, _ ->
            ...
            return framesBytes?.mapNotNull { bytes ->
    ...
    private suspend fun generateImageSequentially(
        ...
        // Ensure files
        ensureFiles(context, onProgress)
        ...
        try {
            val t5File = getFile(context, DEFAULT_T5XXL_ID, DEFAULT_T5XXL_FILENAME)
            ...
            cond = t5Model.precomputeCondition(params.prompt, params.negative, params.width, params.height)
            uncond = if (params.negative.isNotEmpty()) {
                t5Model.precomputeCondition(params.negative, "", params.width, params.height)
            } else {
                t5Model.precomputeCondition("", "", params.width, params.height)
            }
        } finally {
            t5Model?.close()
            t5Model = null
        ...
        try {
            val modelFile = getFile(context, DEFAULT_MODEL_ID, DEFAULT_MODEL_FILENAME)
            val vaeFile = getFile(context, DEFAULT_VAE_ID, DEFAULT_VAE_FILENAME)
            ...
            val bytes = diffusionModel.txt2ImgWithPrecomputedCondition(
                prompt = params.prompt,
                negative = params.negative,
                width = params.width,
                height = params.height,
    ...
    fun cancelGeneration() {
        ...
        // SmolLM doesn't expose cancel explicitly in the wrapper yet, but we can try to close it if needed
        // or just rely on it checking interruption.
    }

    private suspend fun getOrLoadModel(
        context: Context, 
        flashAttn: Boolean,
        onProgress: ((String, Int, Int) -> Unit)?
    ): StableDiffusion {
    ...
    private suspend fun getOrLoadSmolLM(
        ...
        cachedSmolLM?.let { return it }
        
        val finalPath = if (absolutePath != null) {
            File(absolutePath)
        } else {
    ...
    private fun getOrLoadOcrEngine(context: Context): io.aatricks.llmedge.vision.ocr.MlKitOcrEngine {
        cachedOcrEngine?.let { return it }
    ...
    private suspend fun ensureFiles(context: Context, onProgress: ((String, Int, Int) -> Unit)?) {
        // ... Helper to ensure files exist using HuggingFaceHub ...
        HuggingFaceHub.ensureRepoFileOnDisk(context, DEFAULT_MODEL_ID, "main", DEFAULT_MODEL_FILENAME, emptyList(), null, false, true, null)
        HuggingFaceHub.ensureRepoFileOnDisk(context, DEFAULT_VAE_ID, "main", DEFAULT_VAE_FILENAME, emptyList(), null, false, true, null)
        HuggingFaceHub.ensureRepoFileOnDisk(context, DEFAULT_T5XXL_ID, "main", DEFAULT_T5XXL_FILENAME, emptyList(), null, false, true, null)
    }
    
    private fun getFile(context: Context, repoId: String, filename: String): File {
         return HuggingFaceHub.ensureRepoFileOnDisk(context, repoId, "main", filename, emptyList(), null, false, true, null).file
    }
