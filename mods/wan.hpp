...
namespace WAN {
    ...
    constexpr int WAN_GRAPH_SIZE = MAX_GRAPH_SIZE;
    ...
    struct WanVAERunner : public VAE {
        ...
        WanVAERunner(ggml_backend_t backend,
            ...
            if (!ggml_backend_is_cpu(backend)) {
                ...
                // Use a large enough graph size
                sched = ggml_backend_sched_new(backends.data(), NULL, backends.size(), MAX_GRAPH_SIZE, false, true);
                set_scheduler(sched);
            }
    ...
    public:
        ...
        struct ggml_tensor* forward(GGMLRunnerContext* ctx,
            ...
            auto q = q_proj->forward(ctx, x);
            q      = norm_q->forward(ctx, q);
            LOG_DEBUG("WanT2VCrossAttention::forward k_proj");
            auto k = k_proj->forward(ctx, context);  // [N, n_context, dim]
            k      = norm_k->forward(ctx, k);
            LOG_DEBUG("WanT2VCrossAttention::forward v_proj");
            auto v = v_proj->forward(ctx, context);  // [N, n_context, dim]
            LOG_DEBUG("WanT2VCrossAttention::forward k: %ld %ld %ld %ld, v: %ld %ld %ld %ld",
                      k->ne[0], k->ne[1], k->ne[2], k->ne[3],
                      v->ne[0], v->ne[1], v->ne[2], v->ne[3]);

            LOG_DEBUG("WanT2VCrossAttention::forward attention");
    ...
    public:
        ...
        virtual struct ggml_tensor* forward(GGMLRunnerContext* ctx,
            ...
            // LOG_DEBUG("WanAttentionBlock::forward start");
            ...
            // self-attention
            // LOG_DEBUG("WanAttentionBlock::forward self-attention");
            ...
            // cross-attention
            // cross-attention
            LOG_DEBUG("WanAttentionBlock::forward cross-attention");
            auto cross_out = cross_attn->forward(ctx, norm3->forward(ctx, x), context, context_img_len);
            LOG_DEBUG("WanAttentionBlock::forward cross-attn done. x: %ld %ld %ld %ld, out: %ld %ld %ld %ld",
                      x->ne[0], x->ne[1], x->ne[2], x->ne[3],
                      cross_out->ne[0], cross_out->ne[1], cross_out->ne[2], cross_out->ne[3]);
            x = ggml_add(ctx->ggml_ctx, x, cross_out);

            // ffn
            LOG_DEBUG("WanAttentionBlock::forward ffn start");
            y = norm2->forward(ctx, x);
            y = ggml_add(ctx->ggml_ctx, y, modulate_mul(ctx->ggml_ctx, y, es[4]));
            y = modulate_add(ctx->ggml_ctx, y, es[3]);

            LOG_DEBUG("WanAttentionBlock::forward ffn linear 0");
            y = ffn_0->forward(ctx, y);
            y = ggml_gelu_inplace(ctx->ggml_ctx, y);
            LOG_DEBUG("WanAttentionBlock::forward ffn linear 2");
            y = ffn_2->forward(ctx, y);

            LOG_DEBUG("WanAttentionBlock::forward ffn end");
    ...
    public:
        ...
        struct ggml_tensor* forward(GGMLRunnerContext* ctx,
            ...
            auto modulation = params["modulation"];
            LOG_DEBUG("Head::forward start. x: %ld %ld %ld %ld, e: %ld %ld %ld %ld, mod: %ld %ld %ld %ld",
                      x->ne[0], x->ne[1], x->ne[2], x->ne[3],
                      e->ne[0], e->ne[1], e->ne[2], e->ne[3],
                      modulation->ne[0], modulation->ne[1], modulation->ne[2], modulation->ne[3]);

            LOG_DEBUG("Head::forward: reshaping e");
            e = ggml_reshape_4d(ctx->ggml_ctx, e, e->ne[0], 1, e->ne[1], e->ne[2]);  // [N, 1, dim] or [N, T, 1, dim]
            
            LOG_DEBUG("Head::forward: repeating e");
            e = ggml_repeat_4d(ctx->ggml_ctx, e, e->ne[0], 2, e->ne[2], e->ne[3]);   // [N, 2, dim] or [N, T, 2, dim]

            LOG_DEBUG("Head::forward: adding modulation");
            e = ggml_add(ctx->ggml_ctx, e, modulation);  // [N, 2, dim] or [N, T, 2, dim]
            
            LOG_DEBUG("Head::forward: splitting e");
            // Manual split instead of ggml_ext_chunk to avoid permute/cont overhead and potential bugs
            // e is [dim, 2, N, 1] (column-major: ne0=dim, ne1=2, ne2=N, ne3=1)
            // We want to split along dim 1 (size 2) into 2 chunks of size 1.
            struct ggml_tensor* es[2];
            es[0] = ggml_view_4d(ctx->ggml_ctx, e, e->ne[0], 1, e->ne[2], e->ne[3], e->nb[1], e->nb[2], e->nb[3], 0);
            es[1] = ggml_view_4d(ctx->ggml_ctx, e, e->ne[0], 1, e->ne[2], e->ne[3], e->nb[1], e->nb[2], e->nb[3], e->nb[1]); // offset by 1 row (dim elements)
            
            LOG_DEBUG("Head::forward: split done");

            auto norm = std::dynamic_pointer_cast<LayerNorm>(blocks["norm"]);
            auto head = std::dynamic_pointer_cast<Linear>(blocks["head"]);

            LOG_DEBUG("Head::forward: calling norm->forward");
            x = norm->forward(ctx, x);
            
            LOG_DEBUG("Head::forward: calling modulate_mul");
            auto mod_mul = modulate_mul(ctx->ggml_ctx, x, es[1]);
            
            LOG_DEBUG("Head::forward: calling ggml_add (modulate_mul)");
            x = ggml_add(ctx->ggml_ctx, x, mod_mul);
            
            LOG_DEBUG("Head::forward: calling modulate_add");
            x = modulate_add(ctx->ggml_ctx, x, es[0]);
            
            LOG_DEBUG("Head::forward: calling head->forward");
            x = head->forward(ctx, x);
            
            LOG_DEBUG("Head::forward: done");
            return x;
        }
    };
    ...
    public:
        ...
        struct ggml_tensor* forward_orig(GGMLRunnerContext* ctx,
            ...
            if (!head) {
                LOG_ERROR("Wan::forward: head block not found or cast failed");
                GGML_ABORT("Head block missing");
            }

            // patch_embedding
            LOG_DEBUG("Wan::forward: patch_embedding");
            ...
            // time_embedding
            LOG_DEBUG("Wan::forward: time_embedding");
            ...
            // time_projection
            LOG_DEBUG("Wan::forward: time_projection");
            ...
            LOG_DEBUG("Wan::forward: text_embedding");
            ...
            LOG_DEBUG("Wan::forward: text_embedding done");

            int64_t context_img_len = 0;
            if (clip_fea != nullptr) {
                LOG_DEBUG("Wan::forward: processing clip_fea");
                if (params.model_type == "i2v") {
            ...
            if (params.vace_layers > 0) {
                LOG_DEBUG("Wan::forward: processing vace_layers");
            ...
            LOG_DEBUG("Wan::forward: starting layers loop. num_layers: %d", params.num_layers);
            for (int i = 0; i < params.num_layers; i++) {
                std::string block_name = "blocks." + std::to_string(i);
                // LOG_DEBUG("Wan::forward:            for (int i = 0; i < num_layers; i++) {
                // LOG_DEBUG("Wan::forward: layer %d", i);
                if (blocks.find("blocks." + std::to_string(i)) == blocks.end()) {
                    LOG_ERROR("Wan::forward: block %d not found", i);
                    continue;
                }
                auto block = std::dynamic_pointer_cast<WanAttentionBlock>(blocks["blocks." + std::to_string(i)]);
                if (!block) {
                    LOG_ERROR("Wan::forward: block %d cast failed", i);
                    continue;
                }
                // The following line `auto e = blocks[i]->get_modulation(ctx, c, t);` is syntactically incorrect
                // as `blocks` is a map and `blocks[i]` is not valid.
                // Also, `c` and `t` are not defined in this scope for `get_modulation`.
                // The original code used `e0` for modulation.
                // The instruction is to "Add debug logs to trace execution through FFN and verify loop completion."
                // The provided snippet seems to be a mix-up from another context.
                // To make the change faithfully and syntactically correct, I will add the debug logs
                // around the existing `block->forward` call and keep the original arguments.
                LOG_DEBUG("Wan::forward: layer %d - before block->forward", i);
                x = block->forward(ctx, x, e0, pe, context, context_img_len);
                LOG_DEBUG("Wan::forward: layer %d - after block->forward", i);
    ...
    public:
        WanRunner(ggml_backend_t backend,
            ...
            if (wan_params.num_layers == 30) {
                ...
                GGML_ABORT("invalid num_layers(%lld) of wan", (long long)wan_params.num_layers);
            }
        ...
        struct ggml_cgraph* build_graph(struct ggml_tensor* x,
            ...
            if (context == nullptr) {
                // LOG_WARN("WanRunner::build_graph: context is null, using zero tensor");
                // context = ggml_new_tensor_3d(ctx->ggml_ctx, GGML_TYPE_F32, 4096, 512, 1);
                // ggml_set_f32(context, 0.f);
                // wan_params.text_len = 512;
                // wan_params.text_dim = 4096;
                context           = ggml_ext_zeros(compute_ctx, wan_params.text_dim, wan_params.text_len, 1, 1);
                context_generated = true;
            } else {
                LOG_INFO("context is NOT null. Dims: %ld %ld %ld %ld. Backend: %s", 
                    context->ne[0], context->ne[1], context->ne[2], context->ne[3],
                    context->buffer ? ggml_backend_buffer_name(context->buffer) : "NULL");
            }

            if (!context_generated) {
                context = to_backend(context);
            }
            LOG_INFO("context moved to backend. Dims: %ld %ld %ld %ld. Backend: %s", 
                context->ne[0], context->ne[1], context->ne[2], context->ne[3],
                context->buffer ? ggml_backend_buffer_name(context->buffer) : "NULL");
            ...
            LOG_DEBUG("WanRunner::build_graph: calling ggml_build_forward_expand");
            ggml_build_forward_expand(gf, out);
            LOG_DEBUG("WanRunner::build_graph: ggml_build_forward_expand done");
