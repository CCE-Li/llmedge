...
__STATIC_INLINE__ void print_ggml_tensor(struct ggml_tensor* tensor, bool shape_only = false, const char* mark = "") {
    printf("%s (%s): shape(%lld, %lld, %lld, %lld)\n", mark, ggml_type_name(tensor->type), (long long)tensor->ne[0], (long long)tensor->ne[1], (long long)tensor->ne[2], (long long)tensor->ne[3]);
    fflush(stdout);
    if (shape_only) {
        return;
    }
...
__STATIC_INLINE__ struct ggml_tensor* ggml_ext_attention_ext(struct ggml_context* ctx,
    ...
    // LOG_DEBUG("ggml_ext_attention_ext: start. n_head=%ld", n_head);
    // if (q) LOG_DEBUG("q: %ld %ld %ld %ld", q->ne[0], q->ne[1], q->ne[2], q->ne[3]);
    // if (k) LOG_DEBUG("k: %ld %ld %ld %ld", k->ne[0], k->ne[1], k->ne[2], k->ne[3]);
    // if (v) LOG_DEBUG("v: %ld %ld %ld %ld", v->ne[0], v->ne[1], v->ne[2], v->ne[3]);
    ...
    if (!skip_reshape) {
        ...
        // LOG_DEBUG("ggml_ext_attention_ext: reshaping. d_head=%ld, n_kv_head=%ld", d_head, n_kv_head);
    ...
    if (flash_attn) {
        LOG_DEBUG("ggml_ext_attention_ext: flash_attn enabled. L_q:%ld L_k:%ld n_head:%ld C:%ld d_head:%ld N:%ld", L_q, L_k, n_head, C, d_head, N);
        ...
        if (can_use_flash_attn) {
            LOG_DEBUG("ggml_ext_attention_ext: calling build_kqv");
            kqv = build_kqv(q, k, v, mask);
            LOG_DEBUG("ggml_ext_attention_ext: build_kqv done. Checking backend support.");
            if (!ggml_backend_supports_op(backend, kqv)) {
                LOG_WARN("ggml_ext_attention_ext: backend does not support flash attn op");
                kqv = nullptr;
            } else {
                LOG_DEBUG("ggml_ext_attention_ext: backend supports flash attn op");
    ...
    if (kqv == nullptr) {
        LOG_DEBUG("ggml_ext_attention_ext: fallback to default attention. L_q:%ld L_k:%ld n_head:%ld C:%ld d_head:%ld N:%ld", L_q, L_k, n_head, C, d_head, N);
        ...
        kqv = ggml_mul_mat(ctx, v, kq);  // [N * n_head, d_head, L_q]
        kqv = ggml_reshape_4d(ctx, kqv, d_head, L_q, n_head, N);
        kqv = ggml_ext_cont(ctx, ggml_permute(ctx, kqv, 0, 2, 1, 3));  // [N, L_q, n_head, d_head]
        kqv = ggml_reshape_3d(ctx, kqv, C, L_q, N);                    // [N, L_q, C]
    }
    LOG_DEBUG("ggml_ext_attention_ext: done");
...
protected:
    ...
    ggml_backend_buffer_t one_tensor_buffer = nullptr;
    std::vector<ggml_backend_buffer_t> temporary_buffers;
    ...
    void prepare_build_in_tensor_before() {
        ...
        if (ggml_backend_is_cpu(runtime_backend)) {
            one_tensor->data = one_vec.data();
        } else {
            if (one_tensor_buffer == nullptr) {
                one_tensor_buffer = ggml_backend_alloc_buffer(runtime_backend, ggml_nbytes(one_tensor));
            }
            one_tensor->buffer = one_tensor_buffer;
            one_tensor->data = ggml_backend_buffer_get_base(one_tensor_buffer);
            ggml_backend_tensor_set(one_tensor, one_vec.data(), 0, ggml_nbytes(one_tensor));
        }
    }
...
public:
    ...
    virtual ~GGMLRunner() {
        ...
        free_compute_buffer();
        if (one_tensor_buffer != nullptr) {
            ggml_backend_buffer_free(one_tensor_buffer);
            one_tensor_buffer = nullptr;
        }
        free_params_ctx();
        free_compute_ctx();
        if (params_backend != runtime_backend) {
    ...
    struct ggml_tensor* to_backend(struct ggml_tensor* tensor) {
        ...
        if (!ggml_backend_is_cpu(runtime_backend) && (tensor->buffer == nullptr || ggml_backend_buffer_is_host(tensor->buffer))) {
            ...
            ggml_backend_buffer_t buffer = ggml_backend_alloc_buffer(runtime_backend, ggml_nbytes(backend_tensor));
            temporary_buffers.push_back(buffer);
            backend_tensor->buffer = buffer;
            backend_tensor->data = ggml_backend_buffer_get_base(buffer);
    ...
    void compute(get_graph_cb_t get_graph,
        ...
        if (sched) {
            LOG_DEBUG("GGMLRunner::compute: calling alloc_compute_buffer (sched)");
            alloc_compute_buffer(get_graph);
            LOG_DEBUG("GGMLRunner::compute: calling reset_compute_ctx (sched)");
            reset_compute_ctx();
            LOG_DEBUG("GGMLRunner::compute: calling get_compute_graph (sched)");
            gf = get_compute_graph(get_graph);
            LOG_DEBUG("GGMLRunner::compute: calling ggml_backend_sched_alloc_graph (sched)");
            if (!ggml_backend_sched_alloc_graph(sched, gf)) {
                LOG_ERROR("ggml_backend_sched_alloc_graph failed");
                GGML_ASSERT(false);
            }
            LOG_DEBUG("GGMLRunner::compute: ggml_backend_sched_alloc_graph done");
            LOG_DEBUG("GGMLRunner::compute: calling copy_data_to_backend_tensor (sched)");
            copy_data_to_backend_tensor();
            
            // Set threads for CPU backend if present in sched?
            // Assuming runtime_backend is one of them or we don't care for now.
            if (ggml_backend_is_cpu(runtime_backend)) {
                ggml_backend_cpu_set_n_threads(runtime_backend, n_threads);
            }

            LOG_DEBUG("GGMLRunner::compute: calling ggml_backend_sched_graph_compute");
            ggml_backend_sched_graph_compute(sched, gf);
            LOG_DEBUG("GGMLRunner::compute: ggml_backend_sched_graph_compute done");
        } else {
            alloc_compute_buffer(get_graph);
            reset_compute_ctx();
            gf = get_compute_graph(get_graph);
            LOG_DEBUG("Allocating graph...");
            if (!ggml_gallocr_alloc_graph(compute_allocr, gf)) {
                LOG_ERROR("ggml_gallocr_alloc_graph failed");
                GGML_ASSERT(false);
            }
            LOG_DEBUG("Copying data to backend tensor...");
            copy_data_to_backend_tensor();
            LOG_DEBUG("Computing graph...");
            if (ggml_backend_is_cpu(runtime_backend)) {
                ggml_backend_cpu_set_n_threads(runtime_backend, n_threads);
            }
    ...
    void set_scheduler(ggml_backend_sched_t s) {
        sched = s;
    }
};
