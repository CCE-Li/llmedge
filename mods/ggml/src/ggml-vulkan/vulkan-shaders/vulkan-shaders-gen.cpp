...
void execute_command(std::vector<std::string>& command, std::string& stdout_str, std::string& stderr_str) {
    ...
    if (!CreateProcessA(NULL, (char*)cmd.data(), NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
    ...
    if (exit_code != 0) {
        if (stderr_str.empty()) {
            stderr_str = "Command failed with exit code " + std::to_string(exit_code);
        }
    }
#else
    int stdout_pipe[2];
    ...
    if (pid == 0) {
        ...
        if (WIFEXITED(status)) {
            if (WEXITSTATUS(status) != 0) {
                if (stderr_str.empty()) {
                    stderr_str = "Command failed with exit code " + std::to_string(WEXITSTATUS(status));
                }
            }
        } else {
            stderr_str = "Command terminated abnormally";
        }
    }
#endif
}
...
static bool any_compile_failed = false;
...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        string_to_spv("mul_mat_vec_id_" + tname + "_f32_f32", shader, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPE_VEC2", "vec2"}, {"B_TYPE_VEC4", "vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}}));
        string_to_spv("mul_mat_vec_id_" + tname + "_f16_f32_subgroup", shader, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float16_t"}, {"B_TYPE_VEC2", "f16vec2"}, {"B_TYPE_VEC4", "f16vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}}));

        string_to_spv("mul_mat_vec_id_" + tname + "_f32_f32_subgroup_no_shmem", shader, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPE_VEC2", "vec2"}, {"B_TYPE_VEC4", "vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD_NO_SHMEM", "1"}}));
        string_to_spv("mul_mat_vec_id_" + tname + "_f16_f32_subgroup_no_shmem", shader, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float16_t"}, {"B_TYPE_VEC2", "f16vec2"}, {"B_TYPE_VEC4", "f16vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD_NO_SHMEM", "1"}}));
